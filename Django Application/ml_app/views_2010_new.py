...
height = 224  # Replace with your model's input height
width = 224   # Replace with your model's input width
...
def preprocess_image(image_path):
    ...
    image = cv2.resize(image, (height, width))  # Replace with your model's input size
...
class MyDepthwiseConv2D(DepthwiseConv2D):
    def __init__(self, **kwargs):
        # Accept all keyword arguments and pass them to the parent constructor
        super(MyDepthwiseConv2D, self).__init__(**kwargs)

def load_my_model():
    try:
        return keras_load_model('D:/ISB_Final_Hackathon - Sowmya/Deepfake_detection_using_deep_learning/Django Application/models/redeepfake_model.h5', 
                                 custom_objects={'DepthwiseConv2D': MyDepthwiseConv2D})
    except Exception as e:
...
def image_predict(request):
    if request.method == "GET":
        ...
        print(f"Updated uploaded image path: {uploaded_image_path}")
        ...
        if model is not None:  # Check if the model was loaded successfully
            ...
            reconstructed_image = np.squeeze(reconstructed_image)  # Remove batch dimension
            reconstructed_image = (reconstructed_image * 255).astype(np.uint8)  # Convert to 8-bit

            # Save the reconstructed image
            reconstructed_image_path = os.path.join(IMAGE_MEDIA_ROOT, f'reconstructed_{upload_image_file}')
            ...
            context = {
                ...
                'reconstructed_image': os.path.join(settings.MEDIA_URL, 'image_uploaded', f'reconstructed_{upload_image_file}')
            }
            return render(request, 'image_predict.html', context)  # Adjust to the correct template
        else:
