...
def entry_page(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'video':
            ...
            return redirect('ml_app:text_upload')
...
# Function for handling image prediction
def image_predict(request):
    # Load the model and processor inside the image_predict function
    model_dir = "dima806/deepfake_vs_real_image_detection"
    #model_dir = "date3k2/resnet-real-fake-image"
    ...
    if not filename:
        return redirect('image_upload')  # Redirect if no file is uploaded

    # Fetch the file from the upload directory
    file_path = os.path.join(settings.PROJECT_DIR, 'image_uploaded', filename)
  ...
  # Adjust to the correct template
...
#from settings import TEXT_UPLOAD_FOLDER
...
def text_load_model():
    model_path = "models/roberta-base-openai-detector"  # Update the path to point to the models folder
    tokenizer = AutoTokenizer.from_pretrained(model_path, local_files_only=True)
    model = AutoModelForSequenceClassification.from_pretrained(model_path, local_files_only=True)
    classifier = pipeline("sentiment-analysis", model=model, tokenizer=tokenizer)
    return classifier

classifier = text_load_model()  # Load the model when the module is loaded

# Function to parse uploaded file contents
def parse_contents(file, filename):
    try:
        if filename.endswith('.docx'):
            # Process .docx file using docx2txt
            text = docx2txt.process(file)
        elif filename.endswith('.txt'):
            # Read .txt file and decode bytes to string
            ...
            for page in reader.pages:
                extracted_text = page.extract_text() or ''  # Handle NoneType
                text += extracted_text
        else:
...
def classify_text(text):
    ...
    return 50, 50  # Default scores if text is empty

# Function to determine content prediction
def text_get_prediction(real_score, fake_score):
    if real_score > fake_score:
        return "Content is likely generated by a Human."
    else:
        return "Content is likely generated by AI."

# Function to process text prediction
def process_text_prediction(file_path):
    filename = os.path.basename(file_path)

    with open(file_path, 'rb') as file:
        text = parse_contents(file, filename)
        print(text)

    real_score, fake_score = classify_text(text)
    prediction = text_get_prediction(real_score, fake_score)

    return text,real_score, fake_score, prediction
...
def text_upload(request):
    if request.method == 'GET':
        ...
        if 'file_name' in request.session:
            del request.session['file_name']
        return render(request, 'text_upload.html', {"form": text_upload_form})
    else:
        text_upload_form = TextUploadForm(request.POST, request.FILES)
        if text_upload_form.is_valid():
            text_file = text_upload_form.cleaned_data['upload_text_file']
            text_file_ext = text_file.name.split('.')[-1]

            # Check file size limit
            if text_file.size > int(settings.MAX_UPLOAD_SIZE):
                text_upload_form.add_error("upload_text_file", "Maximum file size exceeded.")
                return render(request, 'text_upload.html', {"form": text_upload_form})

            # Validate file extension
            if text_file_ext not in settings.ALLOWED_TEXT_EXTENSIONS:
                text_upload_form.add_error("upload_text_file", "Invalid text file type.")
                return render(request, 'text_upload.html', {"form": text_upload_form})

            # Save the text file in the 'text_upload' folder
            saved_text_file = 'uploaded_text_' + str(int(time.time())) + "." + text_file_ext
            text_upload_path = os.path.join(settings.PROJECT_DIR, 'text_upload', saved_text_file)

            with open(text_upload_path, 'wb') as txt_file:
                shutil.copyfileobj(text_file, txt_file)

            # Store the file path in session
            request.session['file_name'] = text_upload_path
            print(f"File saved: {text_upload_path}")  # Debugging statement

            return redirect('ml_app:text_predict')  # Redirect to the text prediction page
        else:
            return render(request, 'text_upload.html', {"form": text_upload_form})
...
def text_predict(request):
    ...
    if request.method == 'POST' or (request.method == 'GET'):
        ...
        file_path = request.session.get('file_name')
        print(f"Retrieved file path: {file_path}")  # Debugging statement
        print("*********************************3*******************")
        if file_path and os.path.exists(file_path):
            text,real_score, fake_score, prediction = process_text_prediction(file_path)
            print("****************************4****************")
            print(text)
            context = {
            ...
            return render(request, 'text_predict.html', context)  # Render the prediction template
        else:
            return render(request, 'text_predict.html', {"error": "No file uploaded or file does not exist."})

    return render(request, 'text_predict.html')  # For GET requests
