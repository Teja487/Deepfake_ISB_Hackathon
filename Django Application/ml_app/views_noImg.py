...
def image_predict(request):
    ...
    #     print("***********************************************************")
        ...
        if prediction[0, 0] > 0.481 or (prediction[0, 0] <= 0.22):
        ...
        uploaded_image_url = f"image_uploaded/{upload_image_file}"
        uploaded_image_url = uploaded_image_url.replace('\\', '/')
        dest = f"image_predict/image_uploaded"
        shutil.copy(uploaded_image_url, dest)
        ...
        print("Path of image: ", dest)
        print(context)

        return render(request, 'image_predict.html', context)  # Adjust template name accordingly
    else:
        # Handle case where the method is not GET
